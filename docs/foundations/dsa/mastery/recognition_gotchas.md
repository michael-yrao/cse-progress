...
| 2026-08-20 | 239 Sliding Window Maximum (**new**) | — | ⚠️ **Not measurable — spoiled.** The technique was named by another agent before the rep, and the mechanism was then taught in-session. ⚠️ *Row reconstructed Aug 21 from the schedule + `stuck_log.md`; no line was written on the day* — the denominator gap this section exists to prevent |
| 2026-08-20 | 53 Maximum Subarray — **prefix-min method** `R` | — | ⚠️ **Not measurable — first encoding.** The Kadane row is separately 🟢; this was the first time the prefix-min method had been written, so there was nothing to recall. ⚠️ *Row reconstructed Aug 21 from the schedule + `stuck_log.md`* |
| 2026-08-21 | 14 Longest Common Prefix `R` | "the longest possible is min of strs, so we just loop through and compare on shortestString" | ✅ **Hit, pre-code** — names the **bound first** (the answer cannot exceed one chosen string) and then the vertical column scan, which is the whole technique on this problem. ⚠️ **The premise under it was wrong and the learner did not catch it**: `min(strs)` is the **lexicographic** minimum, not the shortest string (`min(["b","aaaa"])` is `"aaaa"`). Code is still correct — a strict prefix of `m` would itself have been the lexicographic min, so the out-of-range read is unreachable — but that argument was coach-supplied and is **not** what they were reasoning from. `min(strs, key=len)` is what the comment describes. ⭐ The learner got most of the way to the proof unaided when asked (*"any lexicographical differences will be caught"*); the missing step was the prefix-implies-smaller contradiction |
| 2026-08-21 | 150 Evaluate RPN `R` | "stack, ideally we store **indices** by default — but since we need to store **calc'd numbers with no indices**, we store the values instead… if not operator push; if operator, pop two, calc, push result" | ✅ **Hit, pre-code**, and the second clause is the interesting one: it is the **739/496 index-vs-value axis applied in the opposite direction** — the learner's default is now *store indices*, and here they justified **not** doing it, because the things on this stack are computed results that have no position in the input. Naming why the habit does **not** apply is a stronger signal than applying it. ⚠️ **Not phase-exit evidence — pre-spoiled** (folder `stack/`, docstring header `Pattern: stack`) and a retry. ⚠️ **Recognition passed, execution did not**: operand order and truncation-toward-zero both coach-flagged (see `stuck_log.md`) — the 901/743/739/853 split again, 6th occurrence |
| 2026-08-21 | **202 Happy Number** `R` (rated rep after the Aug 11 probe) | pre-code comment was *"we need to define an end point / 0,2,3 do not generate a final ending number"* — a hardcoded blacklist, and `{0,2,3}` is not even the real cycle, so the loop never terminates on `n=4` | ❌ **Miss — and the SAME miss as the Aug 11 probe, ten days later.** Both times the pre-code note was a *plan for stopping* rather than a technique, and both times the correct call (cycle detection → a set built during the walk) arrived only after the coach challenged the termination condition. See the write-up below |
| 2026-08-22 | 778 Swim in Rising Water `R` | "Dijkstra — has start/end, moving to the lowest greedily = minHeap; visited set, no adjMap since 2D grid; big diff between BFS and Dijkstra is where we add to visited" | ✅ **Hit, pre-code** — technique named *and* the visited-on-pop discriminator vs BFS volunteered unaided. Half-spoiled (retry; row reads *Dijkstra / Min-Heap*), so not phase-exit evidence — but this is the **first 🟢** for Dijkstra after 0-for-2 in the Aug 10 week. Clean code, correct on both traces |
| 2026-08-22 | 53 Maximum Subarray (**prefix-min method**) `R` | "prefix sum method today; subarray sum = prefix[j] − prefix[i]; to maximize, maximize prefix[j] or minimize prefix[i]; track smallest prefix[i] as we sweep" | ✅ **Hit, pre-code** — named the identity that was **blank on the Aug 20 first-encoding** (came back 🔴 then). The load-bearing half — minimize the earlier prefix while maximizing the current — was stated before any code, and the min-update-after-max ordering (keeps the subarray non-empty) was correct unaided. Half-spoiled (retry) |
| 2026-08-22 | 3 Longest Substring Without Repeating `R` | "dynamic sliding window; set to track what's in the window; variable for longest" | ✅ **Hit, pre-code** — technique + structure named up front. Heavily half-spoiled (retry, folder `sliding_window/`, 🟢 s1), so low recognition value; logged for the denominator. The real note was the **complexity why-clause**: amortized O(n) sharpened from "doesn't re-scan the window" to "l is monotonic" |
| 2026-08-22 | 271 Encode and Decode Strings `R` | "prefix length framing; special char to distinguish current from next; prefix each string with its length" | ✅ **Hit, pre-code** — named the length-prefix scheme, which is the whole design (slice-by-count makes a `#` in the payload a non-issue). Half-spoiled (retry). The miss was on **complexity, not recognition** — encode time O(n)→O(N), see `complexity_gotchas.md` and `stuck_log.md` |
| 2026-08-22 | 211 Add and Search Words `R` | "Trie; searching a plain word is easy but the `.` wildcard means skip the current char and DFS over every child; two params (node + index)" | ✅ **Hit, pre-code** — named the structure *and* the exact thing the wildcard forces (fan out over all children, not index one). Half-spoiled (retry, folder `trie/`). Complexity gate then passed cleanly — O(h·26^d)/O(h) with the 26^d kept for precision. 🟡→🟢 |
| 2026-08-22 | 239 Sliding Window Maximum `R` (🔴 lock-down) | "decreasing monotonic deque, remove from both ends; maxHeap is the O(n log n) baseline we beat" | ⚠️ **Not cold — spoiled Aug 20 + taught.** Technique named correctly, but this is a lock-down of a Blank whose technique was handed over, so not recognition evidence. Execution then took 3 coach-guided fixes (front/back, eviction, off-by-one) — see `stuck_log.md`. → 🟡 |
| 2026-08-22 | 15 3Sum `R` | "sort, then two-sum-sorted per fixed i; set to dedup triplets" | ✅ **Hit, pre-code** — terse but the whole approach (sort + inward two-pointer + set dedup). Heavily half-spoiled (retry, 🟢 s1). The miss was **complexity, not recognition** — space O(n)→O(n²), covered by the freebie → 🟢 s2 |
| 2026-08-22 | 210 Course Schedule II `R` | "topological sort; indegree array + adjMap; enqueue the zero-prereq courses (Kahn's)" | ✅ **Hit, pre-code** — named Kahn's and both structures. Half-spoiled (retry, 🟢 s1). ⭐ **The real win was the complexity gate: O(V+E) shot 3 of 3 HIT** (V kept in both terms, the 133 repair). Note: attributed the no-repeat to the dead `takenCourses` set, not the indegree mechanism → 🟢 s2 |
| 2026-08-25 | 1334 Find the City `R` | "no distinct src/dst, every node is a source, small n → Floyd-Warshall; go through midpoint C to cheapen A→B" | ✅ **Hit, pre-code** — technique + the picking feature (small `n`, all-pairs) named in the top comment. Half-spoiled (retry, row reads *Floyd-Warshall*), so not phase-exit evidence — but the **first 🟢** for Floyd-Warshall, clearing its zero-green. Complexity clean (n³ dominates) → 🟢 s1 |
| 2026-08-25 | 1462 Course Schedule IV (**new**, F-W coverage sibling) | opened on **"Union-Find, find the root parent"** | ❌ **miss** — corrected on one directed failing case (see below). Recovered to Floyd-Warshall/transitive-closure himself, coded clean, complexity clean → 🟡. New problem (nothing labeled), so this **is** recognition evidence: the directed/undirected discriminator was not yet reflexive |
| 2026-08-26 | 57 Insert Interval (**new**) | rejected binary search (wide-merge is O(n) worst case anyway); derived the full overlap complement `end<newStart or start>newEnd` from a counterexample | ✅ **Hit, pre-code** — sorted-interval sweep named unaided. The 🔴 was the *merge mechanics* (three-phase placement of the grown interval), not recognition — approach handed over, see `stuck_log.md` |
| 2026-08-26 | 1584 Min Cost Connect Points — **Kruskal** variant (rated) | "MST → Kruskal → sort all edges by weight + union-find cycle rejection; complete graph so E=V²" | ✅ **Hit, pre-code** — technique + the Kruskal-specific mechanic (global edge sort, not adjacency) named in the top comment. Half-spoiled (variant rep after Aug 19 teach), so not phase-exit evidence, but the teach measured 🟢 a week later. Complexity clean (E log E, E=V²) → 🟢 |
| 2026-08-26 | 435 Non-overlapping Intervals (**new**) | "remove fewest = keep most non-overlapping → sort + greedy + track a frontier" | ✅ **Hit (technique), pre-code** — but the **picking detail missed**: proposed removing "the right one," then "the wider-range one"; the deciding property is the **END** (keep earliest-ending), taught via `[[0,10],[9,11],[10,20]]`. ⭐ **Card the sibling split: max-non-overlapping/scheduling → sort by END (435); merge-all-overlaps → sort by START (56/57). The sort key follows the goal.** → 🔴 (criterion + 3 bugs coach-supplied; learner's call for a 2-day re-rep) |
| 2026-08-27 | 49 Group Anagrams `R` | "map of sorted → unsorted copies" (anagrams share a sorted signature) | ✅ **Hit, pre-code** — the group key named unaided. Half-spoiled (retry, 🟢 s2), so not phase-exit evidence. Complexity clean (n·m log m / n·m) → 🟢 **s3 Graduated** |
| 2026-08-27 | 739 Daily Temperatures `R` | "monotonic stack, scan from the end; store indices → day-count = index difference" | ✅ **Hit, pre-code** — technique + the indices-not-values mechanic named unaided. Half-spoiled (retry, 🟡). The 🟡 was **execution**: the monotone-direction reasoning ("why decreasing") coach-supplied — a *recurring* soft spot — plus an index-vs-temperature pop-test bug (see `stuck_log.md`). Complexity clean → 🟡 |
| 2026-08-27 | 269 Alien Dictionary `R` | "topological graph; prereq counter + adjMap of prereq→unlocks (Kahn's)" | ✅ **Hit, pre-code** — Kahn's + both structures named in the top comment. Half-spoiled (retry, graphs folder + method name). The 🟡 was **execution**: the invalid-order guard used `len(w2)==minLen` (false-fires on equal adjacent words) — coach handed the `["abc","abc"]` counterexample, learner fixed to `len(w1)>len(w2)`. ⭐ Also nailed the **all-unique-letters must appear** modeling point when asked. Complexity clean (O(C) time, O(1) space — fixed alphabet) → 🟡 |
| 2026-08-28 | 57 Insert Interval `R` | "1. insert intervals before that don't overlap · 2. merge intervals that do overlap · 3. insert final merged interval · 4. insert intervals after" (+ the `min(starts)`/`max(ends)` merge, traced off the examples) | ✅ **Hit, pre-code, and it is the structure rather than a label** — the 3-phase sweep and the merge formula were both written before any code, off a blank page. ⭐ **This is the leg that changed since Wed Aug 26**, when the same approach was **coach-handed** on the first exposure; two days later it came back unaided. Half-spoiled on the technique leg (retry, folder `greedy/`), so not phase-exit evidence. ⚠️ **Recognition ≠ execution again**: the call was right and both bugs were boundary conditions inside it — the overlap *test* was half-written (complement of phase 1 rather than the definition) and the bounds guard watched `intervals` instead of `index`. Same split as 332/853 — naming the sweep is not the same as stating when it stops |
| 2026-08-28 | 435 Non-overlapping Intervals `R` | "maximizing non-overlapping intervals is a greedy **sort by end interval** problem — this way, if currentInterval intersects with prior, we can remove it safely" | ✅ **Hit, pre-code, and it is the picking feature, not a label.** *Sort by end* IS this problem — earliest finishing time leaves the most room for what follows, and it is the one clause separating this from sort-by-start (which is right for merging and wrong for scheduling). ⭐ **Coach-supplied on Wed Aug 26; the learner's own two days later.** Half-spoiled on the technique leg (retry, folder `greedy/`). ⚠️ **Recognition ≠ execution, second time tonight**: the call named "prior" correctly and the code bound `prior` to `i-1` — the array predecessor rather than the last **kept** interval. Same shape as 57 an hour earlier and as 332/853 before it — **naming the sweep is not the same as stating what it compares against** |
| 2026-08-28 | 🎯 **PROBE #6 — 637 Average of Levels** (Easy, unseen, label stripped) | written call: "nice little BFS" — then, on being asked for the discriminator: "BFS is literally level order traversal, it makes it nice and clean to group the total by each level and just append at the end of each level" | ✅ **Hit, and the code was correct first pass with no hints** — including the level-size snapshot (`lenQueue` read *before* the inner loop), which is the single thing that breaks this problem. Fully unspoiled: `dsa/probes/`, no pattern in the path, no technique in the docstring. Complexity unaided and for the right reason — **O(n) space because the queue is bounded by the WIDEST LEVEL** (~n/2 in a complete tree), not by a vague "it holds nodes". ⚠️ **But the WRITTEN call was technique-only** — no shape, no picking feature. The discriminator (level-order gives the per-level grouping for free; DFS can do it but must bucket by depth) came out immediately and correctly *when asked*, so the knowledge was there — it just was not in the artifact. **On a probe the written call IS the evidence**; coached on that, rating unaffected. 🟢 **Clean, no tracker row** (probe rule). Probes now stand at **6 run / 1 row** |
| 2026-08-29 | 332 Reconstruct Itinerary (min-heap) `R` | "Eulerian Path since we are not guaranteed to finish at our starting vertex · we want to visit every **edge** once, so that is our visited set · visited = (startVertex, endVertex) but we are **not given those are unique** · minHeap holds the vertices to sort lexicographically · **we might not need visited at all if we are popping off the minHeap — it basically tells us it is visited**" | ✅ **Hit, pre-code, and the strongest version of this call the learner has written.** Three legs, all unaided: the object (Eulerian *path*, and **why** — the endpoint is not guaranteed to be the start), the discriminator (**edges not nodes** — the Eulerian/Hamiltonian split), and the ordering mechanism. ⭐ **The fourth clause is new and is the one that matters**: *popping the heap is itself the visited marker* — i.e. you consume an edge by **deleting** it, which is precisely the bookkeeping that was the whole failure on Aug 14 (`for` + `return` instead of `while` + delete). The parallel-edge observation (duplicate tickets are real, so a `set` of destinations would collapse them) was also the learner's, raised as a clarifying question before coding. Half-spoiled on the technique leg (retry; `graphs/` folder, tracker row reads *Hierholzer*). ⚠️⚠️ **5th CONSECUTIVE recognition-passes / execution-fails on THIS problem — but the split has now narrowed to a single fact.** Everything above was held; what could not be recalled cold was *what to do at the dead end*. Coach supplied one sentence (**the vertex you strand at is the itinerary's last stop**), and the learner produced the post-order + reverse and correct code off it immediately. **Read this as a RETRIEVAL-CUE gap, not a comprehension gap** — the mechanism is understood and re-derivable once named, and four reps of re-coding the loop have not fixed the recall. The drill is the cue (*every EDGE exactly once + lexicographic order → Hierholzer, post-order, reverse*), not the loop. → 🔴 (learner's call over 🟡, for the 2-day re-rep) |
| 2026-08-29 | 133 Clone Graph `R` | "since each node has neighbors, we need to make a **mapping of old to new** · create those nodes first, map them and assign neighbors · we can use **BFS** to assign" | ✅ **Hit, pre-code** — traversal + the clone-identity map, the same two legs as the Aug 9 call, and the *two-pass* structure (create all, then wire) was named before any code rather than discovered while writing. Half-spoiled on the technique leg (retry; folder `graphs/`, method name `cloneGraph`). ⚠️ **Recognition ≠ execution once more**: the call was complete and the single bug was a **guard the earlier reps on this problem already had** — the `None` case, dropped this time; coach supplied the failing input (`adjList = []`), not the fix. → 🟡 (cap forced independently by the 3rd complexity miss on this problem) |
| 2026-08-29 | 155 Min Stack `R` | "**not a true min stack** · more like we are saying do a stack and **also keep track of min** · so what we can do is just **store a tuple in a stack** · lets do **(value, min)**" | ✅ **Hit, pre-code, and the first clause is the whole insight** — *not a true min stack* rejects the framing the problem's own name pushes on you: nothing here is a special stack, it is an ordinary LIFO carrying an **augmented payload**, which is exactly the technique's name in `techniques.yml` (`Stack (augmented entries)`). Naming the payload shape `(value, min)` before writing anything is the mechanism, not a label. Half-spoiled on the technique leg (retry, folder `stack/`). ⭐ **Execution followed the recognition** — correct first pass, no hints, and the empty-stack case handled as a branch inside `push` rather than patched afterward, which is where this implementation usually breaks. That is the opposite of the recognition/execution split running through 332/269/57/435 this month. → 🟢 **s0 → s1, provisional lock-down HELD** |
| 2026-08-29 | 853 Car Fleet `R` (**cold check of the Aug 15 teach**) | "main trick … **if a car is in front it will never be passed** so we have to **sort with highest position first** · speed index is tied with position so we need tuples · **then if any cars catch up to the previous car, they are the same fleet**" | ✅ **Hit, pre-code, all three legs — and the first leg is a physical fact about the problem, not a technique label.** *No car can pass the one in front* is what forces the descending-by-position order; the merge rule and the tuple-pairing obstacle were both named before any code. ⭐⭐ **This is the rep the provisional 🟢 existed to run.** The mechanism was **coach-supplied in a teach on Aug 15**; the Aug 17 rated rep sat two days later and the ledger flagged it at the time as *measuring the teach, not cold recognition* — hence streak 0 and a short lock-down. **Fourteen days on, unaided and correct first pass, ties included.** The teach stuck. Half-spoiled on the technique leg (retry, folder `stack/`). → 🟢 **s0 → s1** |
| 2026-08-31 | 424 Longest Repeating Char Replacement `R` | "sliding window, highest freq char + k is the max size · track maxFreqChar only, not the maxFreq in the window · use r and l to track maxFreq without re-scanning (else O(n²))" | ✅ **Hit, pre-code** — technique + the load-bearing mechanic (the stale, never-decremented `maxFreq`, and *why* it must be stale to stay O(n)) named in the top comment. Half-spoiled on the technique leg (retry, folder `sliding_window/`). ⚠️ **Recognition ≠ execution again**: the call was complete and the bug was the **return value** — `maxFreq + k` overshoots the string on `s="AAAA", k=2`; fixed to a running `max(res, r-l+1)` after one nudge. Cap forced independently by the repeat fixed-alphabet space miss (freebie spent 2026-07-27). → 🟡 (converts 🔴→🟡) |
| 2026-08-31 | 332 Reconstruct Itinerary (min-heap) `R` — ⭐ **cold cue check** (fired label-stripped, before opening the file) | "Eulerian Path — visit each edge once, no guarantee we end at the start · adjMap with minHeap for lexicographic order · the end is a node with no edges left · **no visited set — popping the heap consumes the edge** · post-order append, reverse at the end" | ✅✅ **Cold hit, and the retrieval-cue gap has closed.** All four legs written before any code, unaided — including the exact mechanism (post-order + reverse + heap-pop-is-the-marker) that was **coach-supplied Aug 29** and had blanked five reps running. Note ᴬ's cap ("if the hinge must be supplied again, the cap binds") **did not trigger** — nothing was supplied. Code correct first pass on the greedy-strands case. Complexity total correct (O(E log E) / O(E)), one self-corrected itemization (build heappush is O(log E), not O(1)). → 🟢 **provisional (s0, +10 lock-down)** — first non-🔴 rating this problem has ever had. ⚠️ Half-spoiled on the technique leg (retry, `graphs/` folder), so not phase-exit evidence on its own; but the drill (*every EDGE once + lexicographic → Hierholzer, post-order, reverse*) is what the Aug 29 note prescribed and it landed |
| 2026-08-31 | 261 Graph Valid Tree (Union-Find) `R` | "union-find; 2 checks: all nodes connected + no cycle · exactly `n-1` edges lets us skip the connectivity sweep — `len(edges)==n-1` + no cycle ⟹ connected" | ✅ **Hit, pre-code** — technique + both tree conditions + the load-bearing shortcut (edge-count check replaces the connectivity pass) all named before code. Half-spoiled (retry, `graphs/` folder). Code correct first pass, no hints; complexity clean and itemized (O(V+E) time, O(V) space, UF op O(α(n)) treated as O(1)). → 🟢 **s1 → s2 (+60)** |
| 2026-08-31 | 355 Design Twitter `R` | "followeeMap (who follows whom) · timestamp + minHeap for the 10 most recent · tweetMap person→(timestamp, tweetId)" | ✅ **Hit, pre-code** — all three stores + the merge named before code; getNewsFeed correctly trims the min-heap to 10 (oldest fall out) then reverses for newest-first. Half-spoiled (retry, `heap/` folder). Correct first pass, no hints. ⭐ Complexity **capped-heap insight all theirs** — heap ≤ 10 ⟹ each op O(log 10) = O(1), so getNewsFeed is O(n·t) not O(n·t·log(total)); space sharpened to object-footprint itemization (coach-added, not a miss). Latent smell flagged (class-var `timestamp`, works but reads as shared state) — learner moved it to `__init__`. → 🟢 **s1 → s2 (+60)** |
| 2026-09-01 | 239 Sliding Window Maximum `R` | "monotonic **decreasing** deque, max at the front (queue side); maxHeap is the O(n log n) baseline we beat" | ✅ **Hit, pre-code** — technique + picking feature (only need the current window's candidates, so no full ordering → beat the heap) named up front. Extended discussion resolved the learner's own front/back naming clash (their comments described the mirror image; the physical order is front→back decreasing, front = max). Half-spoiled (retry, `stack/` folder). ⚠️ **Recognition ≠ execution, third straight month of it**: three execution bugs coach-flagged — (1) append gate silently dropped the current index instead of pop-all-smaller-then-**always**-append, (2) read max off the back `[-1]` not the front `[0]`, (3) eviction bound off by one (self-caught). Complexity clean (amortized O(n) / O(k)). → 🟡 (no conversion) |
| 2026-09-01 | 150 Evaluate RPN `R` | "push numbers, on an operator pop two and push the result; never push operators" | ✅ **Hit, pre-code**, and this time recognition *and* execution both landed — correct first pass, no hints. Half-spoiled (retry, `stack/` folder). ⭐ The load-bearing decision was made unaided: `int(x/y)` (float divide then truncate) instead of `//`, which is the one 150 trap (`//` floors toward −∞, spec wants truncate-toward-zero: `6 // -132 = -1` vs the required `0`). Complexity itemized correctly (O(n) / O(n), ~n/2 operands worst case). → 🟡 → 🟢 s1 (+30) |
| 2026-09-01 | 53 Maximum Subarray — prefix-sum method `R` | "subarray sum = prefix[j] − prefix[i]; maximize prefix[j] while minimizing prefix[i]; naive all-pairs is O(n²), one pass beats it" | ✅ **Hit, pre-code** — named the technique, the algebraic identity, and priced the naive baseline it improves on. Half-spoiled (retry + method named in the tracker row + `greedy/` folder). Clean execution: `len+1` prefix with `prefix[0]=0`, and `maxSum` updated **before** `prefixI` so the subtracted prefix is always strictly earlier (non-empty subarray). Complexity clean (O(n)/O(n)). Provisional s0 lock-down HELD → 🟢 s1 (+30) |
| 2026-09-01 | 271 Encode and Decode Strings `R` | "prefix length framing; pick any separator; decode slices by count so a `#` in the payload is a non-issue" | ✅ **Hit, pre-code, both methods** — recognition and execution both landed, correct first pass no hints. Half-spoiled (retry, `arrays_and_hash/` folder). ⭐ Complexity gate the whole point (fired on both per note ᴰ): **the Aug 22 encode-time miss (O(n)→O(N), scales with total characters not string count) is FIXED**. One sharpen on decode space — called it O(len(string)) for the transient slice, but that slice *is* the appended output element, so auxiliary space is O(1) (a copy that becomes the answer is counted once, in the output). Precision note, not a conceptual miss → no cap. → 🟡 → 🟢 s1 (+30) |
| 2026-09-04 | 84 Largest Rectangle in Histogram `N` — unseen, first exposure | "find the next smaller height on each side — that's the wall; monotonically increasing stack" | ⚠️ **Assisted, not cold.** Opened by transferring Container-With-Water's two-pointer `min(h)×(r−l+1)` — the right *instinct* (reach for a neighbour) but it collapses on the interior min, and the learner saw that (`[2,1,5,6,2,3]` → 6 not 10, "extra height of 1 at index 1"). Reached the real call only after being walked through the **pin-vs-track** reframe (fix each bar as the limiting height, expand to the nearest shorter bar each side = nearest-smaller-element). ⚠️ Over-scaffolded en route — described the mechanism and prompted "name it" (logged, [[feedback_recognition_gate]]). Final call (monotonic increasing stack) was theirs. Half-spoiled (`stack/` folder). **→ 🔴 (never-encoded, not decayed — §2a).** Every execution piece (width formula's two `-1`s, why `stack[-1]` is the left wall, the end-flush/`append(0)` sentinel) had to be unpacked; complexity clean & unaided (O(n)/O(n)). Durable frame landed in the learner's words ("monotonic stack = loop + while-that-restores-order + a per-problem personal-best at the pop"). Re-rep Sep 6 measures whether the recognition fires without the two-pointer detour. |
| 2026-09-04 | 1462 Course Schedule IV `R` — ⭐ recognition **re-measure** (Aug 25 was a MISS: opened Union-Find) | "2D reachability array so queries are O(1); this is just Floyd-Warshall (transitive closure), especially given numCourses ≤ 100" | ✅ **Hit, pre-code — MISS CONVERTED.** Aug 25 opened Union-Find (undirected-connectivity instinct on a directed-reachability problem) and corrected; this time went straight to transitive-closure Floyd-Warshall in the top comment, unaided. Half-spoiled (retry, `graphs/` folder + method in tracker name) — but the *call* is exactly what note ᴷ flagged to re-measure, and it landed. Code correct first pass (triple loop `left→middle ∧ middle→right ⟹ left→right`), no hints. Complexity clean & itemized (O(n³) triple loop / O(n²) matrix; query + edge-seed passes subsumed). → 🟡 → 🟢 s1 (+30). Picking feature to card: **directed + is-u-ancestor-of-v reachability → transitive closure (Floyd-Warshall / DFS-per-node), NOT Union-Find** (UF answers undirected *connectivity*, loses direction). |
| 2026-09-05 | 55 Jump Game `N` — unseen, first Greedy exposure | "greedy — jump to the highest value index we can reach" → corrected to "**highest reach `i+nums[i]`, not highest value**" → frontier reframe | ⚠️ **Partial.** Greedy family theirs. The **picking feature was wrong**: proposed jumping to the highest *value* in range (should be furthest *reach*), and stayed in a path-*simulation* frame — coded a hop-and-repeat loop that infinite-looped (candidate scan included `i` itself, no stall-exit). Reach-vs-value and the **frontier** reframe (one pass, track furthest reachable; `if i > maxReach: return False`) both coach-supplied. Learner then wrote the clean one-pass form and named the shift themselves (*"track the frontier, not the traveler"*). Complexity O(n)/O(1) unaided. → 🟡 (first exposure). Picking feature to card: **linear reachability / min-jumps asking yes-no or an optimum (not the path) → track a running frontier in one pass, don't simulate jumps.** |
| 2026-09-05 | 332 Reconstruct Itinerary (pre-sorted adjacency) `R` — Hierholzer 2nd form | "Eulerian Path — visit each edge once, not each node; no guarantee we end at start; pre-sort each adjacency list for lexicographic order" | ✅ **Recognition clean, cold** — Eulerian + edges-not-nodes + no visited-node-set + self-derived that pre-sorting handles lexical order, all pre-code. Half-spoiled (retry, `graphs/` folder). ⚠️ **Recognition ≠ execution once more**: iterative-stack Hierholzer did not come naturally, execution taught (peek/push/pop-when-stuck/reverse hand-trace). Learner resolved the pop-direction subtlety themselves (`reverse=True` so smallest sits at list end for O(1) `pop()`). → **teach, unrated**; rated re-measure Sep 15. Not phase-exit evidence (execution supplied). |
| 2026-09-05 | 2097 Valid Arrangement of Pairs `N` — unseen, Hierholzer 2nd problem (drill pulled forward from Sep 7) | "graph → Eulerian path → Hierholzer's; the numbers are nodes and the pairs are edges; the issue is finding the starting node" | ✅ **Recognition clean, cold** — Eulerian/Hierholzer named unaided, and the learner *spotted the one difference from 332 themselves* (no given start). ⚠️ **But the SAME two-axis split, twice in one day**: modeling (pair=edge not node) + degree-based start + the iterative Hierholzer loop all had to be supplied — the loop notably did **not** transfer from the morning's 332 teach (kept writing a plain edge-non-consuming DFS across ~3 rounds). → **🔴 (never-encoded execution), unrated drill**; the iterative-Hierholzer re-measure Sep 15 covers this and 332 together. Picking feature already solid; the gap is executing Hierholzer from memory, not recognizing it. |
| 2026-09-07 | 2097 Valid Arrangement of Pairs `R` — ⭐ **execution re-measure** of the Sep 5 drill | "numbers are nodes, pairs are edges; Eulerian Path vs Circuit; find start via degree (leaving −1, entering +1, start = the −1 node, else pairs[0][0]); Hierholzer with stack + adjMap; output built after Hierholzer finishes" | ✅✅ **BOTH axes landed — the two-day-old split CLOSED.** Recognition clean (half-spoiled retry) as before, but this time **the iterative Hierholzer that had to be supplied ~3 rounds on Sep 5 was written cleanly from a blank page** — peek-top / pop-to-result-when-stuck / else-push-and-consume-edge, reverse, then rebuild edges from consecutive nodes. Correct first pass, no hints. Complexity itemized & correct (O(V+E)/O(V+E); one tiny label slip — `resultNodes` called O(V), it's O(E)=one node per edge — dominated, no cap). → **🟢 s0 provisional (+10 lock-down)**; unblocks 753 (`green:2097`). The Hierholzer execution gap is now proven, not supplied — the Sep 15 332 lock-down is the remaining half. |
| 2026-09-07 | 57 Insert Interval `R` — Intervals cluster | "sharing any point merges; 3 phases — before (newStart > curEnd) / merge (newEnd ≥ curStart, min-start max-end) / after" | ✅ **Hit, pre-code, both axes.** Half-spoiled (retry, `greedy/` folder). The load-bearing piece is the **3-phase decomposition** stated before any code — each phase one loop, one stop condition — and the merge criterion (`newEnd ≥ curStart`) named correctly. Clean execution, no hints; complexity itemized (O(n) single partition-pass / O(1) aux). → 🟢 s1 (+30). Greens the Intervals zero-green cluster. |
| 2026-09-07 | 435 Non-overlapping Intervals `R` — Intervals cluster | "keep as many as possible → sort by end; track latestEnd, drop the interval if latestEnd lands inside its range" | ✅ **Recognition hit, pre-code** — the exchange-argument greedy (sort by end, keep earliest-ending) was named unaided. Half-spoiled (retry, `greedy/` folder). ⚠️ **Execution bug coach-flagged, not self-caught**: overlap test read `latestEnd > start AND latestEnd < end` — the second clause is inert (sorted by end ⟹ latestEnd ≤ end) except on **equal ends**, where it flips false and keeps an overlapper (0 on `[[1,2]×3]`, want 2). Overlap is start-only: `latestEnd > start`. Complexity clean (O(n log n) sort-dominated / O(n) Timsort aux). → 🟡 (no conversion). |
| 2026-09-07 | 912 Sort an Array (Merge Sort) `R` | "merge sort; divide and conquer together" | ➖ **Fully spoiled — not recognition evidence** (technique named in both the tracker row and the method name). Logged for the denominator only. Rep was an **execution** slip: two D&C bugs (half-open boundary drop; merge bound `minLen`) coach-flagged + a space-complexity miss. → 🟡. |
| 2026-09-08 | 45 Jump Game II (**new**, Greedy intake 2/6) | "jump to the node with `i+nums[i]` highest possible" — greedy, max-reach, written pre-code | ✅ **Hit on the family + picking feature (greedy, maximize next reach), pre-code** — the sequel-to-55 instinct landed. ⚠️ **Not clean phase-exit evidence — family pre-spoiled** by the `greedy/` folder + docstring `Pattern: greedy` (the 150/155/739 scaffold-path defect, on a *new* problem). ⚠️ **Recognition ≠ execution again**: the call was right but the window mechanics were the gap — collapsed `left` to the single jump point → infinite loop (coach handed `[1,2,1,1,1]` + "what should left/right hold"; learner derived `left = old_right+1`), plus an `n=1` edge returning 1 not 0 (coach handed `[0]`). Complexity bounds correct (O(n) amortized / O(1)), new-problem freebie; the monotonic-advance *why* was sharpened by coach. → 🟡. Picking feature to card: **min-jumps / fewest-steps over a reach array → BFS-layer frontier (each layer = [prev_end+1, farthest]), not a per-node greedy hop.** |
| 2026-09-08 | 133 Clone Graph `R` | "old to new map with bfs/dfs and then remap" — two-pass, pre-code | ✅ **Hit, pre-code** — the two-pass (clone-all-nodes then wire) + old→new identity map, same call as Aug 9/Aug 29. Half-spoiled (retry, `graphs/` folder). ⚠️⚠️ **Rated 🟢 by LEARNER OVERRIDE against the rubric** — coach proposed 🟡. Two execution bugs coach-flagged, not self-caught: (1) BFS loop iterated `node.neighbors` (fixed start) instead of `currentNode.neighbors` → only radius-1 nodes cloned, KeyError in the wiring pass on `[[2,4],[1,3],[2,4],[1,3]]`; (2) missing `None` guard → `None.val` on empty input, a **repeat** of the Aug 29 miss on this exact problem. Learner's reason: *"got bored half way, wasn't fully into it, wasn't challenging."* Complexity clean & cold (O(V+E)/O(V) — the historical V-drop miss repaired). **Greens 133's zero-green (6 reps), 🟡→🟢 s1 (+30 → 2026-10-08).** ⚠️ **Watch the +30 check**: if it slips, the two flagged bugs are the reason the 🟢 was soft. |
| 2026-09-11 | 🎯 **PROBE #7 — 547 Number of Provinces** (Medium, unseen, label stripped — first Medium probe) | cold: *"feels like a literal Floyd-Warshall (a↔b, b↔c ⟹ a↔c) — build the closure, then count with **Number of Islands** grid-DFS"* | ❌ **Miss, cold, fully unspoiled — the grid trap, and the cleanest components-vs-islands miss the ledger holds.** The model error: treated an **adjacency matrix as a spatial grid**, walking `(row±1, col±1)` neighbours — so the count was wrong (`[[1,0,1],[0,1,0],[1,0,1]]` → returned 5, answer 2). The correct model (**connected components → Union-Find / DFS over a node's row**) was coach-walked over several turns (failing case → "a cell is an *edge* not a pixel" → node `i`'s neighbours are row `i`'s 1s → DFS carries transitivity by *walking* it, so no closure needed). ⭐ **Learner's own post-mortem is the durable card:** the statement's transitivity clause *reads* like Floyd-Warshall but is just the **definition of a connected component** (an equivalence relation) — present in *every* components problem, so it is **not** an FW cue; the title ("Number of Provinces" = count the groups) is the real signal. **Picking feature to burn in: Islands = neighbours are the spatial up/down/left/right CELLS; Provinces/components = neighbours are the node's ROW entries. Reach for FW only when the all-pairs closure/distances is the OUTPUT, never for a group *count*.** ⭐ **The learner's own sharpest test (stated next):** *no weighted edges + nothing to minimise ⟹ the whole weighted shortest-path family (FW/Dijkstra/Bellman-Ford) is out before you even reach "components → UF."* A connectivity/count question needs no path costs; the "how far" family answers a question this problem never asked. Execution once pointed at UF was clean and theirs (find w/ path compression, union by rank, decrement-per-successful-merge — the `union()`-return gate held). Complexity: time O(n²) correct ("UF is free" — amortized α(n)); **space missed O(n²)→corrected O(n)** (both maps keyed by node = n entries; the n² is the *input* matrix, not allocated). **→ 🔴 (recognition supplied), earns a tracker row.** Probes now **7 run / 2 rows** (202, 547). ⚠️ First Medium probe → the ratchet is satisfied; next probe stays Medium. |
| 2026-09-13 | 1489 Critical & Pseudo-Critical Edges in MST (**new**, Kruskal 2nd form) | pre-code, self-written: *"MST, so Prim's or Kruskal — but we're specifically saying some edges are pseudo-critical, so we need to be able to **ban specific edges**, thus it has to be UF and Kruskal"* | ✅ **Hit on the picking feature, pre-code and unaided.** Family half-spoiled by the `graphs/` folder + `Pattern: graphs`, but the load-bearing discrimination — **Kruskal-over-Prim's because you must exclude/force a chosen edge**, which UF makes trivial — was generated cold and is the exact cue this problem turns on. ⚠️ **Recognition ≠ full solve (the recurring split):** the call was right, but the problem's *own definition* (what a pseudo-critical edge is → the force-inclusion test that separates it from a useless edge) had to be **taught + Socratically drawn out**, and execution took 3 coach-caught bugs (see stuck_log). So: technique-selection solid; the gap was the problem-specific reasoning + execution, not the recognition. → 🟡. **Card: "must ban/force a specific edge" ⟹ Kruskal + UF, never Prim's** (Prim's grows from a seed and can't cleanly exclude a named edge). |
| 2026-09-16 | 547 Number of Provinces (probe #7 **+2 re-rep**, recognition re-measure) | cold, pre-code comment: *"union find"* — went straight to UF, no Floyd-Warshall, no grid | ✅✅ **MISS CONVERTED — the Sep 11 grid trap is gone.** Called **Union-Find cold and unaided**; the whole FW/all-pairs detour and the adjacency-matrix-as-spatial-grid model that made #7 a 🔴 did not reappear. The learner's own Sep 11 card (transitivity clause = *definition of a connected component*, not an FW cue; count → components) held. ⚠️ **Recognition ≠ execution (the recurring split):** two coach-surfaced bugs — (1) the union loop dropped the `isConnected[i][j]` guard, unioning **every** pair (surfaced via a hand-trace prompt, then self-caught: *"oh right i didnt even check if it is 1"*); (2) a `n = len+1` phantom node compensated by a `-1` on the return — self-canceling, correct output but a muddy node→component model, defended then cleaned to `n = len` / plain return. Complexity clean, both dims, α(n) justified by path-compression + union-by-rank. → **row DROPPED at learner's call** (recognition gap — the row's whole reason — is closed; residual is plain UF, broadly 🟢/🎓). File returned to `dsa/probes/` (rowless disposable). Probes now **7 run / 1 tracked row remains** (202; 547 retired, 648 stands). |
| 2026-09-13 | 56 Merge Intervals `R` — Intervals cluster (**blind rep**, priors stashed) | pre-code: *"sort by beginning interval, then merge if start ≤ priorEnd, else continue"* | ✅ **Hit, pre-code, both axes — from pure recall.** Retry with prior attempts physically stashed, so this measured recall not reading. Sort-by-start + the merge criterion (`start ≤ priorEnd`, extend end to `max`) named before any code; clean execution all cases incl. nested + unsorted-start. Complexity O(n log n)/O(n) correct, and the space was **credited to the sort** (Timsort O(n)) — the exact term the 2300/row-98 trap says people drop. → 🟢 s1 (+30). **Greens 56 out of the Intervals zero-green cluster.** |
...
- **2026-08-21 · 202 Happy Number (Seen-Set)** — ❌ **miss, and a repeat of this problem's own Aug 11
  miss.** The pre-code comment (*"we need to define an end point / 0,2,3 do not generate a final ending
  number"*) is a **blacklist**, not a technique — and the constant is wrong on its own terms: the
  unhappy cycle is 4·16·37·58·89·145·42·20, so `{0,2,3}` catches only the literal inputs 2 and 3 and
  the loop runs forever on `n=4`.
  ⭐ **The learner diagnosed the SHAPE of the error unprompted** — *"I feel like I am chasing a black
  list but that shouldn't be how I approach this problem"* — before any correction was offered. That is
  the half worth keeping: they could not yet name the technique, but they could tell that a
  looked-up-constant solution was the wrong kind of answer.
  ⚠️ **Then googled mid-rep** and returned with *"unhappy numbers repeat after 8 iterations"* — which is
  the same failure one level down: another constant to hardcode, and still not the method. Named as
  such at the time; it fed the 🟡.
  **What actually unlocked it** was the bounded-range argument, which the learner had to be walked to in
  three steps: each value has exactly one successor · the worst legal input (9,999,999,999) maps to 810
  in one step · so from step two everything lives in 1…243. Given that, *"instead of a static set, it
  should be updated each iteration"* was theirs.
  ⭐ **Durable form, and it is a general move rather than a fact about this problem:** to bound an
  operation's output, **plug in the worst legal input and look at where it lands** — do not try to
  derive the bound. Squaring digits destroys magnitude because the output scales with the digit
  *count*, which is log of the value.
  **Nearest neighbour NOT reached, and deliberately not named by the coach:** the same detection with
  O(1) space. It is the queued `Floyd Fast/Slow` variant on this row — see the hold note below.

⚠️ **The `Floyd Fast/Slow` variant's trigger has fired and is HELD.** `techniques.yml` queues it on
