...
- **2026-08-22 · 271 Encode and Decode Strings · 🟡 Shaky** — code clean/correct (length-prefix `len#str`, slice-by-count so a `#` in the payload is a non-issue), but **encode time given as O(n) (word count) and defended with "append is O(1)"**. The trap: `.append(string)` stores a *reference* in O(1), but the later `"".join(...)` reads every character → O(len). Durable form: **producing an N-char output is Θ(N) regardless of how few append calls it takes** — count characters written, not operations. Decode complexity (O(N)/O(N)) was correct unaided. Also left a debug `print` in `decode`.

- **2026-08-22 · 239 Sliding Window Maximum · 🟡 Shaky** (🔴 Blank lock-down from Aug 20) — technique recalled (decreasing monotonic deque, both ends), but **three execution bugs, coach-guided to fix**: (1) read `deque[-1]` for the window max — **front/back inversion, a REPEAT of the Aug 20 teach bug**; in a *decreasing* deque the front `[0]` is the max, back `[-1]` the min. (2) evicted by `len(deque) > k` — the deque holds only candidates, not the whole window, so length ≠ window size; evict by **index**: front is stale when `i - deque[0] + 1 > k` (span exceeds k). (3) off-by-one (`> k` vs `>= k` / the `+1` span form). Learner surfaced (1) and (2) by questioning their own code; (3) was coach-caught. Complexity clean (amortized O(n), each index in/out once; O(k) space). **Monotonic Deque 🔴→🟡 — still no 🟢.**

- **2026-08-23 · 127 Word Ladder · 🟡 Shaky** — code clean (level-BFS, wildcard adjacency, correct recognition, self-cleaned a debug `print`). Sticking point was purely complexity: **string-cost repeat** — dropped the per-string `m` (word-length) factor, counting *how many* keys/patterns (`n·m`) but not that each is an `m`-char string, so gave `O(n·m)` where it's `O(n·m²)` for both the adjMap build and space. Phase-oscillated from the Aug 13 miss (that time build right / BFS wrong; today BFS `O(n·m²)` unprompted, build + space dropped). Self-corrected under cues (*"each key is m chars"*, *"why is build m but BFS m·m when you slice the same way?"*). Fix: **either stay symbolic `O(n·m²)` or collapse `m≤10` to `O(n)` explicitly — never the inconsistent middle.**
...
- **2026-08-28 · 57 Insert Interval · 🟡 Shaky** (was 🔴 Aug 26, when the approach itself was handed over — this time the **3-phase decomposition, the `min/max` merge and the whole structure were the learner's, off a blank page**). Two coach-supplied fixes, both boundary conditions rather than conceptual gaps:
  - **Half an overlap test.** Loop 2 guarded on `interval.end >= newInterval.start` only — the *complement of loop 1*, not the definition of overlap. The missing half is `interval.start <= newInterval.end`. Without it the condition can never go false (merging only lowers `newInterval[0]` while ends rise to the right), so loop 2 **swallows the entire rest of the array**. Failing case was the file's own example 2: `[12,16]` merged into `[3,10]`, then ran off the end.
  - **Bounds guard on the wrong variable.** All three loops indexed `intervals[index]` with no `index < len(intervals)`. First fix was `while intervals and ...` — which tests *"is the list empty?"*, answered once and never again, while `index` is the thing that moves. Fixed the empty case only; `[[1,2]] + [5,6]` and both docstring examples still raised `IndexError`. Coach had to name that the guard was watching the wrong variable.
  - **Complexity:** time O(n) unaided and correctly itemized (`index` never resets, so each interval is touched once across all three loops). Space took one nudge — answered "no additional space usage", which is false as stated since `result` is allocated; recovered to the right frame once asked to name the distinction: **O(1) auxiliary, excluding the O(n) required output.**
  - **Durable form (learner asked for a better mental model — this is the one that landed):** don't test for overlap, **build the intersection and see if it is real**. Intersection is `[max(starts), min(ends)]`; it exists iff `max(starts) <= min(ends)`. The mirror is what makes it stick — **merge/union is `[min(starts), max(ends)]`**: AND tightens (higher floor, lower ceiling), OR loosens. ⚠️ And the catch that is load-bearing in the code: **intersection of two intervals is always an interval; union is not.** `[min,max]` is the true union only when they touch, so the overlap test is the precondition that makes the merge honest — the original bug was exactly "took a union of two disjoint sets and called it one interval".
  - **SQL anchor (learner's request, landed well):** an interval is a `BETWEEN` predicate. Intersection = `AND` = `GREATEST(starts) <= LEAST(ends)` — literally the double-booking join `a.starts_at <= b.ends_at AND b.starts_at <= a.ends_at`. Union = `OR`, which does **not** collapse to one range; Postgres refuses it outright (`ERROR: result of range union would not be contiguous`) and makes you say `range_merge()` if you want the hull. Merging a whole list is the **gaps-and-islands** pattern with a running `MAX(end)`.
  - **Self-corrected afterwards, unprompted:** simplified loop 2 back to the single clause `intervals[index][0] <= newInterval[1]`, which is correct for the stronger reason — loop 1 guarantees the dropped clause on entry, *and* it cannot flip mid-loop since `newInterval[0]` only shrinks while `intervals[index][1]` only rises. Verified against a reference merge on 9 edge cases + 3000 random inputs: all pass.

- **2026-08-28 · 435 Non-overlapping Intervals · 🟡 Shaky** (was 🔴 Aug 26, when the greedy criterion itself was coach-supplied). **The criterion was the learner's this time** — *"maximizing non-overlapping intervals is a greedy sort by end interval problem"*, pre-code and unaided, which is the whole problem. Two coach-flagged bugs:
  - **Swapped arguments at the call site.** Helper declared `overlap(priorIntervalIndex, currentIntervalIndex)` but was called `overlap(i, i-1)` — so `prior` held the *later* interval. The parameter names then lied, which is why re-reading the helper body never revealed it. ⚠️ **The symptom was total, not partial**: with the arguments swapped the test becomes `prior.start < current.end`, and since the list is sorted by end ascending (`prior.end <= current.end`) plus `prior.start < prior.end` always, it is **unconditionally true** — every input returned `n-1`. Learner initially reasoned about the *intended* binding (*"currentStart is 2 and priorEnd is 2"*) and needed the actual substitution laid out (`overlap(1,0)` → `prior=[2,3]`, `current=[1,2]` → `1 < 3`).
  - **Compared against the array predecessor, not the last KEPT interval** — the conceptual miss, and the one that caps this at 🟡. The loop tested `intervals[i]` against `intervals[i-1]` even when `i-1` had just been counted as removed. Failing case `[[1,2],[1,5],[3,6]]` → returned `2`, answer `1`: `[3,6]` was compared against the discarded `[1,5]` instead of the kept `[1,2]`. **The learner's own pre-code comment said "prior" and meant the right thing; the code implemented "prior" as `i-1`.** Fix was theirs once the trace was handed over — named the invariant (*"we need to keep track of the latest actual interval"*), correctly reduced it to carrying **only the end value**, and inlined the helper away.
  - **Complexity clean and unaided, including the non-obvious half:** O(n log n) time / **O(n) space, both bound by the sort**, loop subsumed by each. ⭐ Naming the sort's temporary buffer is the part usually missed here — the common wrong answer is O(1) from counting only the declared variables.
  - **Durable form:** in a sort-by-end interval greedy, the comparison anchor is **the last interval you kept**, never the previous array element. The two coincide only when nothing has been removed yet, which is exactly why the bug survives the docstring examples and dies on the first removal followed by a non-overlap.
  - Verified: 7 hand cases (incl. negatives and touching endpoints) + 20,000 random inputs against a reference sweep, all pass.
  - ⚠️ **Intervals (sort + sweep) now has three problems and all three are 🟡 — still zero-green**, so it stays a conversion target rather than moving to coverage work.
...
### 2026-09-20 · 78 Subsets · 🟡 (backtracking intake 2/2, first exposure, no primer)

Recognition cold + correct (backtracking; "all possible subsets"), and the 5-question template recalled from 22 (Sat) one day later — path/state/choice/validity(N/A)/base-case — applied to a new problem unaided. That recall is the win the phase measures. Execution leaned on coach: (1) base case `state > len(nums)` off-by-one, fixed to `==` via a `[1]` trace; (2) the path-management cluster — `path.append()` returns None (so `option` was None), missing `return` after recording the base case (falls through to an out-of-bounds index), and copy-vs-reference aliasing (`result.append(path)` stores a live reference that later mutation empties) — all three coach-diagnosed. Learner fixed with a copy-per-branch variant (fresh `path.copy()` down each choice; valid, sidesteps append/pop undo). Complexity: space O(n) correct/itemized (depth n + path ≤ n); time initially O(2ⁿ), corrected to **O(n·2ⁿ)** on prompt (2ⁿ leaves × O(n) copy). ⚠️ Re-rep 2026-09-30 watch item: mutation/copy semantics and the append/pop undo shape (the canonical single-path variant) — the copy-per-branch form worked but hid the undo beat the phase wants owned.

### 2026-09-20 · 1552 Magnetic Force Between Two Balls · 🔴 (re-seat of the Sep 13 max-boundary teach)

**The re-seat's question — is the Sep 13 "binary search on the answer / max-boundary" template writable now? — answered 🔴: not yet.** The final code is correct (verified ex1→3, ex2→999999999), but almost every structural beat was coach-supplied or corrected, not recalled.

- **Came back unaided:** the bounds (`left=1`, `right=position[-1]-position[0]`) and the *label* "this is max boundary" (written in a comment).
- **The core framing was NOT recalled.** Opened by trying to bisect **positions** — "place a ball at start and end, then the middle, find the closest bucket" — i.e. divide-and-conquer on placement, the exact trap the problem is built on. The reframe to *binary-search the answer value (the minimum gap), not positions* was coach-supplied. This is the load-bearing idea of the whole pattern and it didn't survive from Sep 13.
- **Direction inverted.** First wrote the *min-boundary* form (`if achievable: right = middle`) despite labeling it max-boundary — would hunt the *smallest* achievable force (~1). Corrected to `left = middle` / `right = middle - 1` with the round-**up** midpoint `(left+right+1)//2` once flagged (the round-up rationale — `left=middle` stalls otherwise — also coach-surfaced).
- **`canAchieveForce` greedy scaffolded** (the 3 decisions: sort → place leftmost → place when `gap ≥ d` → count vs `m`; why leftmost = leaves most room). Then two bugs: `latestBasket` never updated after placing (every basket compared to `position[0]`) — coach-caught; and the missing **`position.sort()`** — learner had misread the problem (thought the array index carried meaning; it's an unordered set of coordinates on a line), clarified then sorted.
- **Complexity:** space O(n) (Timsort) correct/itemized. Time initially O(n log n) counting only the sort, then O(log **n**) for the search (conflating array length `n` with the value range `D`). Corrected over two prompts to **O(n log n + n log D)**, D = `position[-1]-position[0]` up to 1e9 — the search runs log D iterations, independent of n. (This is the canonical binary-search-on-answer complexity trap.)

⚠️ **Re-rep 2026-09-22 (Tue) watch items, in priority order:** (1) does the *search-on-answer* framing fire cold (or does bisecting positions return); (2) max-boundary direction + round-up midpoint from blank; (3) the greedy `canAchieve` including the `latestBasket` update and the sort. The template is the whole rep — recognition of *which* template plus its direction is where this fell down, not the greedy detail.

### 2026-09-22 · 1552 Magnetic Force Between Two Balls · 🔴 → 🟡 (re-rep of the Sep 20 re-seat)

Sep 20 watch items (1) and (2) both came back cold: the pre-code comment framed it as **binary search on the answer, max-boundary**, sorted first, and wrote `l = mid` / `r = mid - 1` with the round-up midpoint unaided. The sticking point is now one thing seen three times: **what quantity `l`/`r`/`mid` stand for.** (a) `canAchieve` walked every integer coordinate `range(1, position[-1]+1)` rather than the baskets (places balls where there are none; O(1e9) per check), coach-caught. (b) Bounds regressed from Sep 20: `l = position[0]` (a *coordinate*) where the search space is *gaps* (`l = 1`); `[5,6], m=2` returns 5 instead of 1, because a max-boundary search only raises `l`. Coach-explained with the index / coordinate / gap split. (c) Complexity: sort O(n log n) right, but binary search first priced at log n. Corrected on prompt to **O(n log n + n log k)**, k = `position[-1] − position[0]`. Space O(n) Timsort, itemized. ⚠️ Oct 2 watch item: **name the unit of `l`/`r` before writing them.**

### 2026-09-22 · 901 Online Stock Span · 🟢 s1 → 🟡 (decay after a month)

Sticking point: **what to do with the popped prices.** Framed it as "a traditional stack loses the detail to populate 4 for 75" and asked where to store it. The fix — push `(price, span)` so a pop hands its already-absorbed days forward (`span += priorSpan`) — came after a coach trace hint (70 had already returned 2). Also asked *why decreasing*: the stack keeps the nearest **higher** price to the left (the one that stops the count); anything ≤ today is absorbed. Code clean after. Complexity: space named both per-call O(1) and object O(n) (called O(n) "trivial" — it's the object footprint); time given as O(n)/call, amortized **O(1)** reached on the "how many times can one price be popped?" prompt (the Aug 13 card). Oct 2 watch item: **the span-carrying tuple, cold.**

### 2026-09-23 · 39 Combination Sum · 🔴 (backtracking intake 1/2, first attempt)

The backtracking plan template (path/state/choice/validity/base case) came back unaided, the third problem in a row where it did (22 → 78 → 39). The shape was theirs. What didn't come back is **what makes 39 different from 78**:

- **Uniqueness bound missing.** `state = n/a?`: nothing stopped `[2,2,3]` / `[2,3,2]` / `[3,2,2]`. Coach hint: "if picks must follow `candidates` order, once you've picked 3 can you pick 2 again? 3 again?" The learner then derived "sort, and state = the smallest value allowed next." (Sorting isn't required. It enables an early `break`.)
- **Two templates mixed.** The comment said "choose or not choose" (the take/skip version), but the code was the pick-next loop, with a skip call added (`backtrack(path.copy(), smallestState)`, the same arguments again, so it recursed forever). The bound was also never advanced (`smallestState` passed through unchanged). Both bugs were coach-caught. The learner said the loop version "doesn't make too much sense" and asked for the take/skip version. Coach explained the two templates: **the loop's next iteration IS the skip; a take/skip call inspects one index and makes exactly 2 calls, and the skip call must move the index.**
- **Take/skip (v1) built with guided questions:** state = index; base case = out of bounds (learner's own); take → same index (reuse), skip → index + 1. It's correct. v2 was fixed to pass `number` as the next bound and is also correct.
- **Complexity (taught on request, not rated):** d = target ÷ smallest candidate (depth). Pick-next is **O(n^d · d)** time (n-way branching, O(d) `sum`/copy per call) and O(d) stack. Take/skip is O(2^(d+n) · d).

⚠️ **Sep 25 re-rep watch items:** (1) name *why* duplicates occur and what state prevents them, before coding; (2) pick ONE template and say how it skips. For the loop, the next iteration is the skip. For take/skip, the skip call moves the index; (3) advance the bound/index on the take call.

### 2026-09-23 · 846 Hand of Straights · 🟡 → 🟡

Count map + lowest-first + sort were the learner's own. Needed help with **why lowest first is safe** (the smallest card left can only start a group: 1 in `[1,2,3,2,3,4]` has no 0 to sit behind), shown with a coach position table, and a nudge on where the order comes from (a sorted/heap structure alongside the map). Coach-caught bug: `range(start, groupSize)` → `range(start, start + groupSize)` (groups too short; starts ≥ groupSize "succeeded" without using up cards). Complexity miss (not rated, queued): the marking loop is O(n) in total, not O(n·k). Oct 3 watch item: **say the only-starts-a-group reason before coding; write the group bound as start + size.**

### 2026-09-23 · 1489 Find Critical and Pseudo-Critical Edges in MST · 🟡 → 🟡

The critical/pseudo-critical definitions, carrying the index through the sort, the union-find from scratch (correct, with `numComponents`), both exclude and force-in helpers, and the "forced-in weight == base weight" pseudo test were the learner's own. The coach supplied the Kruskal-over-Prim reasoning (asked for), and it named the exclude/force-in mechanism. Coach-caught: (1) excluding a bridge edge yields a forest with a *lower* weight, so a connectivity check is needed; (2) the disconnected sentinel was `-inf`, which must be `+inf` to compare as "more expensive"; (3) critical edges also pass the pseudo test and must be excluded. Complexity miss (not rated, queued): 2E+1 builds, so O(E²). Oct 3 watch item: **state the leave-out / force-in comparisons unaided, and cover the disconnected-graph case.**

### 2026-09-25 · 743 Network Delay Time (Dijkstra — array-scan) · 🟡 → 🟡

Capped at 🟡 before coding (learner's own call): the Sep 24 partial attempt had already taken the dense-Prim's analogy as a hint. Recognition comment, distance/visited arrays, adjacency map, min-scan helper, relax helper, and the unreachable → -1 check were all the learner's own, and the code passed all six cases first run. Coach-supplied today: the outer-loop shape (one settle per pass, `for _ in range(n)`, the `inf` scan result as the exit) with a hand trace of Example 1, and on request the why behind settling the min (non-negative edges + every path exits the visited set through an unvisited node). Complexity: space O(V+E) itemized correctly; time O(V²) right number, wrong attribution (scan omitted, relaxation priced as V·E) — carded, not rated. Oct 5 watch item: **state the loop shape unprompted**, and price the scan and the relaxation as separate terms.

### 2026-09-25 · 55 Jump Game · 🟡 → 🟡

Shape (yes/no reachability via `i + nums[i]`) was the learner's own; the technique was never named in the comment and the learner flagged the block themselves: "very hard to not think about how we do the jump." Forward attempt tracked `furthest_jump` but never checked whether index `i` itself was reachable — coach-surfaced with `[3,2,1,0,4]` (returns True at i=4 via 4+4). Learner found the forward idea unintuitive and chose to work backwards; the coach supplied the backward loop's shape (goal starts at `len-1`, walk right-to-left, move the goal to `i` when `i + nums[i] >= goal`). Code then correct on 8 cases first run. Complexity O(n) / O(1) right. Oct 5 watch item: **name the technique and the single tracked value unprompted**, and for whichever direction is chosen, say what "unreachable" looks like in that value before coding (forward: `i > furthest`; backward: `goal != 0`).

### 2026-09-25 · 46 Permutations · 🆕 → 🟡 (backtracking intake 2/2, first attempt)
