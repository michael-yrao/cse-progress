...
| 208 Implement Trie | **design-object footprint, and the wrong parameter (space)** | Per-call space correct for all three methods. Two gaps. **(a)** Refined `search` from O(n) to **O(h)** — backwards: the loop is `for char in word`, so it runs ≤ `len(word)` however tall the trie is. Tight bound is `O(min(n,h))`; **O(n) is the one to state**, since the key must be processed. **(b)** The **object footprint** was absent after an explicit cue, which is a regression — the per-call/whole-object split had been volunteered cold on 146 and 155. ⭐ **The learner then improved on the stock answer**: told the object is `O(N·L)`, they pushed back that it *"will almost never be the case"*. Correct — `O(N·L)` is attainable (26 distinct first letters) so it is tight, but it prices the case where the structure is pointless. Exact answer is **O(P), P = distinct prefixes**, and `P ≪ Σ|wᵢ|` is precisely why a trie is worth using | 2026-08-16 | **spent** — first miss on this problem, so **no cap** |
