...
| 226 Invert Binary Tree | **space**: gave O(n) but justified it with the *time* reason ("we invert every node"). Space is the **recursion stack = O(h)**, worst-case O(n) on a skewed tree, O(log n) balanced. Number right, attribution wrong — tree-height space category | 2026-09-14 (learner-requested follow-up) | **2026-09-20 ✅ space cold re-ask clean** — O(h) recursion stack, correctly attributed (exactly the category missed). |
| 226 Invert Binary Tree | **time**: gave **O(h)**, should be **O(n)** — invert swaps *every* node, so work = n; height only bounds stack depth (the space term), not total work. Mirror of the space slip — traversal *shape* mapped onto the wrong axis. Corrected cold; re-verify next close-out | 2026-09-20 | |
| 912 Sort an Array (Merge Sort) | **space**: called it **O(n log n)** from "the stack calls" — two errors. (1) stack depth is **O(log n)** (halving), not n log n; (2) the stack isn't even dominant — the `merge` result arrays + `nums[l:r]` slices are. The O(n log n) is **total allocated over the whole run**; **peak live** (what space complexity measures) is **O(n)**, since each merge's inputs are freed as it returns. Needed a full **peak-vs-total** re-teach, not a nudge | 2026-09-17 | **2026-09-20 ✅ cold re-ask clears** — bound O(n), avoided the O(n log n) total-vs-peak trap. Attribution still needed a nudge: the O(n) is the merge arrays (peak-live, inputs freed on return), the call stack is O(log n) — learner pinned the O(n) on the stack. |
| 130 Surrounded Regions (BFS) | **time**: O(m·n) is **unconditional** (two full-grid sweeps — border-seed pass + flip pass), NOT "from the all-O worst case" (that reasoning is the *space* bound — the queue holds O(m·n) only when the grid is one big region). Attribution swapped time↔space. Self-flagged at rating Sep 19, queued then | 2026-09-19 | **2026-09-20 ✅ cold re-ask clean** — time O(m·n) attributed to the unconditional full sweep, space O(m·n) to the worst-case queue; the time↔space swap now mapped correctly. |
...
| 648 Replace Words (Trie prefix search) — **RECURRENCE** | **time**: gave the sentence scan **O(D·S)** (D = total root chars, S = total sentence chars). One `prefixSearch(word)` walk is ≤ len(word) dict-lookup steps — bounded by min(len(word), longest root), never by D — so the scan sums to **O(S)** and total time is **O(D + S)**. Space O(D + S) was right and itemized (trie ≤ D nodes; split + result ≤ S). Self-corrected on one non-leading nudge. Sep 14 reached the same O(L1+L2) only after tightening, so this is the second time the query cost got multiplied by the structure size | 2026-09-24 | |
...
| 424 Longest Repeating Char Replacement | fixed-alphabet array (space) | O(n) → **O(1)** (freqMap ≤ 26 keys — uppercase-only constraint) | 2026-07-27 | **spent → REPEAT MISS 2026-08-31 (capped that rep at 🟡)**: gave O(n) space again; same ≤26 collapse missed. Time O(n) was itemized correctly (amortized inner `while`). **⟶ 3RD MISS 2026-09-10**: gave O(n) space a third time; same ≤26 collapse. No cap consequence this time — the rep was already 🔴 on execution (never-encoded, algorithm coach-supplied; see stuck_log). Time O(n) correct again. **⟶ 4TH MISS 2026-09-12** (+2 re-measure): gave O(n) again — but this time, under one cue, **stated the ≤26 bound himself**, then rationalized the loose bound as *"safer to say O(n) without that constraint."* Progress on *seeing* the bound; the gap is now a **convention error** — analyze against the GIVEN constraint, so the tight answer is O(1) and the hedge reads as missing it. Coach corrected the framing (O(1) here; O(min(n,Σ)) only if the alphabet were unbounded). Capped the rep at 🟡 (freebie long spent). Time O(n) itemized correctly with the amortized inner `while` |
...
| 127 Word Ladder (BFS) | **string cost (time AND space)** — *new category, see above* | BFS phase `O(V + V·L)` → **`O(V·L²)`** (the loop re-slices exactly as the build does, so the phases tie); space `O(V·L)` → **`O(V·L²)`** (`V·L` keys × `L` chars each). ⭐ **The build phase was priced CORRECTLY and unprompted** — `O(C·L)` with the slicing named — so the category was in hand and then dropped twice downstream. ⭐ **Also volunteered: the fixed-alphabet collapse** — a `.it` bucket holds ≤26 words, which kills the `E` term outright (6th of that family: 242/567/424/269/621). ⚠️ **Separate finding — convention drift:** `L` was held symbolic for time and then collapsed via `L ≤ 10` for space, in the same answer. Pick one and defend it; mixed conventions are indefensible under a follow-up | 2026-08-13 | **spent → REPEAT MISS 2026-08-23 (capped that rep at 🟡)**: same dropped-`L` (here `m`) factor, **phase-oscillated** — Aug 13 priced the build correctly and dropped the BFS; Aug 23 the reverse (BFS `O(n·m²)` unprompted, build given as `O(n·m)`, and space `O(n·m)` too). Both m-drops caught only under cues (*"each key is m chars"*; *"why is build m but BFS m·m when you slice the same way?"*), then self-corrected to `O(n·m²)`/`O(n·m²)`. ⚠️ **Convention-drift finding recurred too:** gave the inconsistent middle `O(n·m)` — kept one `m`, dropped the other with no justification — instead of either symbolic `O(n·m²)` or an explicit `m≤10` collapse to `O(n)`. The category is *close* (each phase gets priced right in isolation) but not yet stable across both phases in one pass. **⟶ 3RD MISS 2026-09-02 — cap WAIVED to 🟢 by learner override (recorded, not silent), same argument as the 323 precedent below:** space given as `O(N·L)`, self-corrected to `O(N·L²)` under one cue (*"how many keys, each how long?"*). Recognition + code were clean off a blank page, no hints. Learner's case: *"we've failed on this quite a few times already and writing the same code is not going to help and is only slowing down my overall progress — mark it clean and find a way to clean up the complexity problem."* The unit that decays is the **category** (string-is-not-O(1)), not 127's code, which is over-learned; re-repping the code measures recall that was never in doubt. **📌 PINNED RESOLUTION — decouple the concept from the coding rep via a cold *complexity probe* drill** (mirror of recognition probes): fire a solution's code, learner gives time+space cold, no writing; tracked here with a real hit/miss denominator; front-load the recurring families (string-is-not-O(1), fixed-alphabet collapse, recursion-stack depth, holds-vs-touches). Disposable — clean probe = no row, only a miss re-drills. **Shape + cadence to settle at the Sep 7 build (not built yet).** |
| 648 Replace Words | **query cost independent of structure size (time)** — a trie lookup costs the *key's* length, not the trie's (the 211/14 shape) | O(D·S) → **O(D + S)** — build touches every root char once; each sentence word walks ≤ its own length, so the scan is ≤ S | 2026-09-24 | n/a (retired) |
...
| 912 Sort an Array (Merge Sort) | **peak simultaneous space vs total allocation over the run** (space only; time `O(n log n)` was correct) | Gave space **O(n log n)**, reached by multiplying the O(n) merge work by the O(log n) recursion depth. That product is the **total array memory allocated across the whole run** — but each level's arrays are freed as the merge returns, so it never all coexists. **Peak** (what "space complexity" means) is **O(n)**: the held `leftSide`s down one stack path sum to n/2+n/4+… = n, plus the current `result` (≤ n) and the O(log n) call stack. ⭐ **Durable cue: space is the *high-water mark of what's alive at one instant*, not the sum of everything ever created.** Sibling of the worst-case-vs-steady-state category (150) — both are "which snapshot of the run do you measure." First miss on 912 → freebie. (Rating was already 🟡 this rep from two execution bugs, so no additional cap applied.) | 2026-09-07 | spent |
| 472 Concatenated Words (**new**) | **had no idea — asked to be taught** (not a miss; carded so the post-139 rep is asked cold) | Learner could not state either axis and asked to be taught. **Taught the transferable method — `time = states × work-per-state` for any memoized recursion** — plus trie-build O(C). Applied: build O(C); per-word DFS memoized on `(index,count)` = O(L²) states × O(L) walk = O(L³); total **O(C + N·L³)** time (count-free/index-only caching would be O(N·L²)); space **O(C + N·L²)** (trie + persistent `lru_cache(None)` keyed on word). ⚠️ **Did not land** — *"not yet, will get it at DP."* `states × work` is core DP machinery → re-drill when 1D DP opens (Oct 12). Rep was 🔴/phase-gated (Word Break never encoded); complexity taught, not tested. | 2026-09-09 | **1 of 2** *(new-problem double freebie)* |
