...
| Medium | [323. Number of Connected Components (Union-Find)](https://neetcode.io/problems/count-connected-components) | `green:323-DFS` + `surplus>=1` | **Variant rep, moved off Sun Aug 16, 2026** when the day was trimmed to the 8.0 DSA ceiling. Held as a **state condition rather than a date**, per the Waiting Room rule: it is not an overdue review, it is a third method on a problem whose DFS and BFS rows are both already 🟢. Pulling it while the Aug 17 week already sits ~17.5 units against ~14.1 of headroom would just move the overflow. |
