...
| Easy | [496. Next Greater Element I](https://leetcode.com/problems/next-greater-element-i/) | 🟢 | 2 | 2026-11-18 | 2026-09-19 | 2026-07-04, 2026-07-06, 2026-08-06, 2026-08-16, 2026-09-19 |
| Easy | [206. Reverse Linked List (Iterative)](https://leetcode.com/problems/reverse-linked-list/) | 🎓 | 3 | 2027-03-18 | 2026-09-19 | 2026-04-23, 2026-05-26, 2026-06-12, 2026-06-20, 2026-07-20, 2026-09-19 |
...
| Medium | [721. Accounts Merge (Union-Find)](https://leetcode.com/problems/accounts-merge/) | 🟡 | 0 | 2026-09-27 | 2026-09-17 | 2026-07-30, 2026-08-09, 2026-09-08, 2026-09-17 |
...
| Medium | [3650. Minimum Cost Path with Edge Reversals](https://leetcode.com/problems/minimum-cost-path-with-edge-reversals/) | `surplus>=1` | **Dijkstra consolidation rep (5th problem).** Learner's pick Sep 26, 2026 from the company-wise pull (`pull_interview.py --company Google,Amazon,Meta,Microsoft,Apple --technique "Shortest Path"`, 23 eligible): Meta 23.4 · Google 20.9 · Amazon 15.7 · Microsoft 7.3 — the only Medium on all four lists. ⭐ **Why this one:** plain sum-Dijkstra where the test is the **modeling** — augment the graph with reversed edges at 2× cost before running the standard loop — the step 743/778 never exercise. Target: seat at the **Sep 28 build**, scaffold at kickoff. Number/premium verified against LeetCode GraphQL Sep 26. |
| Hard | [1368. Minimum Cost to Make at Least One Valid Path in a Grid](https://leetcode.com/problems/minimum-cost-to-make-at-least-one-valid-path-in-a-grid/) | `surplus>=1` | **Dijkstra consolidation rep (6th problem) — the 0/1-weight variant.** Learner's pick Sep 26, 2026 from the same pull: Google 31.1 · Meta 8.0 · Amazon 7.2 — the highest single-list frequency of any pure-Dijkstra candidate. ⭐ **Why this one:** edge weights are 0 or 1, so the recognition call is **0-1 BFS (deque) vs Dijkstra (heap)** — a new flavour, not another distance grid. Hard → active-block slot. Target: seat at the **Sep 28 build**, scaffold at kickoff. Verified Sep 26. |
| Medium | [1514. Path with Maximum Probability](https://leetcode.com/problems/path-with-maximum-probability/) | `surplus>=1` | **Dijkstra consolidation rep (7th problem) — the multiplicative variant.** Learner's pick Sep 26, 2026 from the same pull (`pull_interview.py --company Google,Amazon,Meta,Microsoft,Apple --technique "Shortest Path"`, 23 eligible): Google 25.4 · Microsoft 12.0 · Amazon 5.0 · Meta 5.0 — on all four big lists. ⭐ **Why this one:** relaxation **multiplies and maximizes** instead of adding and minimizing — the product-flavour contrast the study guide named in July; reverses the Sep 20 swap that dropped it for 1102. ⚠️ Three pulled siblings (3650 · 1368 · 1514) now compete for next week's ≤2 consolidation slots — the Sep 28 build seats two, holds one. Target: **Sep 28 build**, scaffold at kickoff. Number/premium verified against LeetCode GraphQL Sep 26. |
...
| ⭐ | Hard | [224. Basic Calculator](https://leetcode.com/problems/basic-calculator/) | `Stack (expression evaluation)` — **no-🟢 and thin (1/2)**. Also settles the open Aug 11 objection that 150 had no value: its defence was *"it's the base rung under 224/227/772"*, which only holds if something above it is scheduled | `green:150` — ⚠️ **`rated:150` FIRED Aug 21 and was HELD** (learner: *"150 isn't green yet, no 224 yet"*). The rep came back 🟡, so `Stack (expression evaluation)` still has **no 🟢**; a Hard second rung on an unproven base manufactures another 🟡. Re-stated as a state condition so it cannot expire silently — next chance is 150's Aug 31 review |
...
| **Advanced shortest-path — bidirectional search · Johnson's** | bidirectional BFS: [127. Word Ladder](https://leetcode.com/problems/word-ladder/) · Johnson's: *(Codeforces — no clean LC)* | **Bidirectional** search grows a frontier from both ends and meets in the middle (far fewer expansions on a large branching factor). **Johnson's** = all-pairs shortest path *with negative edges* (Bellman-Ford reweighting + per-source Dijkstra). The accepted interview answer stays plain BFS / Dijkstra / Floyd-Warshall; these only prune constant factors or cover the rare negative-APSP case. (**A\*** lives in the "Famous-but-low-ROI" bench in [`study_guide.md`](../study_guide.md).) |
