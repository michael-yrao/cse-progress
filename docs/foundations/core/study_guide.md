...
Every Sunday, open `docs/foundations/core/dsa_progress.md` and sweep for all problems whose `Next Review Date ≤ end of the coming week`. Slot them into the upcoming schedule before filling active blocks or new content. Use this priority order for warmup slots:
...
### 📋 Post-Warmup Updates:
Log the result in `docs/foundations/core/dsa_progress.md` using the comfort system:
