...
- **Start-of-day kickoff:** on "start today" / "start my day" / `/start-day`, run the daily routine — backlog check → read today's schedule → scaffold *only* today's coding + template-fill items (new = new file, retry = append dated banner; **no-code warmups and blind sprints get nothing**) → present the plan. Idempotent within a day. (skill draft §2a)
