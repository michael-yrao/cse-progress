...
# ── Attempt · 2026-08-31 ──────────────
class Twitter_20260831:
# need a map to keep track of who user is following so that is a followeeMap
# 10 most recent, so that means we need a timestamp and keep track of that so minHeap
# we also need to keep track of posts for each person so tweetMap of person -> timestamp,tweets

    def __init__(self):
        self.followeeMap = collections.defaultdict(set)
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        minHeap = []
        # for each person this userId follows, get their feed
        for person in self.followeeMap[userId]:
            for time, tweet in self.tweetMap[person]:
                heapq.heappush(minHeap,(time,tweet))
                while len(minHeap) > 10:
                    heapq.heappop(minHeap)
        # return reverse of minHeap
        result = []
        while minHeap:
    ...
    def follow(self, followerId: int, followeeId: int) -> None:
        if followerId not in self.followeeMap[followerId]:
            self.followeeMap[followerId].add(followerId)
