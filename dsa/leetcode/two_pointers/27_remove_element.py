...
class Solution:
    ...
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        # if val is found, we check if i+1 is out of bound
        # if not out of bound, check if nums[i] == nums[i+1], if not, increment counter
        # otherwise just continue
        # remove all elements after index counter-1
