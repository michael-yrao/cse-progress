...
class Solution:
    ...
    def singleNonDuplicate_20260906(self, nums: List[int]) -> int:
        ...
        # annoyance here is obviously the duplication
        # so what we should do is set some ground rules
        # 1. when we move to a number, it should be at the first index of the number
        # 2. if there were no single numbers, above index is always even
        # what this means is if we are at an even number, the numbers beneath it cannot possibly be the results

        l, r = 0, len(nums) - 1

        while l < r:
            m = (l + r) // 2
            # check if m is on the right index
            if m%2!=0:
                m-=1
            # we are at even index and equal to next value so everything before is dups
            if nums[m] == nums[m+1]:
                l = m + 2
            else:
                r = m
