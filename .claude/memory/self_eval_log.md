...
## 2026-09-24 [P1] fam: chained-commit-push — `git commit && git push` in one command shipped a stale showcase.json/big-o.json past report-only ERRORs
**What:** The close-out ran commit and push as one chained command. The pre-commit hook's `export_showcase.py` / `export_bigo.py` steps printed `ERROR:` lines (1334's showcase symbol `findTheCity_20260825` was in the cut-short stash, not the file; bigo auto-picked the empty `_20260924` stub), and because the hook runs them `|| true` the commit went through and the push followed before the output was read. Net: origin/main carried a `showcase.json`/`big-o.json` stale for 648 and 743 (line anchors moved when today's attempts were added) — the exports never regenerated because the 1334 error aborted the write.
**Why:** Two causes. (1) Report-only checks only work if someone reads the report before the irreversible step; chaining removed the reading moment. (2) The cut-short stash state (`retry-and-restore.md` calls it normal) orphans any `showcase.yml` symbol that lives in the stashed slice — a design interaction nobody had hit because no cut-short retry had been a showcased pick before.
**Fix/ladder:** Landed now: 1334 reverted to its pre-session file + stash dropped (fresh scaffold at Fri kickoff), exports regenerated, both `--check` green, follow-up commit pushed. Rung 3 landed: CLAUDE.md gate 8 now says commit and push are two commands with the hook output read between. **Deferred to the learner (explicit, not re-proposed):** rung 1 — make an `ERROR:` from `export_*.py --check` BLOCK the pre-commit (drop the `|| true`; `decisions.yml` `showcase-contract` currently says report-only, so it is a decision), and have `restore_history.py` warn when a kept stash holds a `showcase.yml`/`bigo.yml` symbol. Decide at the Fri Sep 25 kickoff or the Sep 27 close-out.

## 2026-09-24 [P1] fam: commit discipline — a directory-level `git add` swept another session's in-progress Big-O rework into a pushed commit; main was broken for ~5 minutes
**What:** On "commit and push" for the Quiz refactor (site repo), I staged `src/app/features/games` and `src/app/features/quiz` as whole directories. A second session was concurrently reworking Big-O Trainer in the same working tree; its new component/html/deck/spec files under `quiz/big-o/` went into commit `74cdffd`, but the untracked model and service they import did not — the pushed main could not build. Repaired with two index-only commits (`7a16345`, `6620096`) that restore the released Big-O files at the new path without touching the working tree, then verified the head builds in a detached temp worktree.
**Why:** I printed `git status --short` in the same command that ran the `add` and `commit`, so the staged list was never inspected before the commit existed. And "the tree was clean when the engineer started" was treated as still true at commit time, twenty minutes later, with another session known to be active in the same checkout.
**Fix/ladder:** Habit (rung 4), first occurrence. When a working tree has changes I did not author: stage by explicit file list taken from the engineer's report, never by directory; run `git diff --cached --stat` as its own call and read it before the commit call; re-check `git status` at commit time, not at brief time. If it recurs, promote to `feedback_commit_discipline.md` as a pre-commit sweep step.

## 2026-09-25 [P1] fam: cross-machine sync — the dotfiles lapse recurred (Mac 09-24, Windows 09-25); source fix landed
**What:** Windows `~/.claude` was 2 commits behind claude-dotfiles with a harness-rewritten `settings.json`, the mirror image of the Mac the day before. Nothing pulled the repo on either machine, and the harness's permission churn guaranteed a conflict on every manual pull. Second occurrence in two days -> rung 1: `~/.claude/hooks/dotfiles_sync.py` (SessionStart, global settings.json) now fetches, auto-commits `settings.json` churn, merges `origin/main` with a three-way JSON merge when only `settings.json` conflicts, pushes, and reports one line per action; dirty rule/agent/hook files are reported, never touched. 34 hook tests pass incl. a real-git end-to-end merge. Not covered: a machine whose `~/.claude` was never restored (no `settings.json` wiring -> the hook cannot fire); that needs a canary in the project repo + a bootstrap script — both landed the same day (`global_layer_canary.py` in cse-progress, `bootstrap.py` in claude-dotfiles, verified by a real run into a temp home). Caught in review: my brief A had the canary's fix line point at `~/.claude/bootstrap.py`, a file that by definition does not exist on the machine the banner fires on; corrected to clone-then-run before landing.
**Why:** the restore recipe and the "commit + push ~/.claude too" obligation lived as prose in an opt-in memory file (rung 4) — exactly the tier the ladder says cannot hold a rule that must fire unprompted.

## 2026-09-25 [P2] fam: brief hygiene — a "live smoke test" step in an engineer brief committed and pushed an unreviewed settings.json edit
**What:** The dotfiles_sync brief told the engineer to run the real hook against `~/.claude` as its smoke test, after wiring the hook into `settings.json`. The hook did what it is built to do: committed the dirty `settings.json` and pushed it plus the pending merge commit — before the tech lead had reviewed anything. The engineer flagged it correctly. Push was the learner-authorized outcome anyway, but the wiring edit reached origin unreviewed.
**Why:** the brief's verification step had a side effect on the real repo, and its ordering (wire, then run) was written assuming a clean tree that the brief's own deliverable 4 would dirty.
**Fix/ladder:** Habit (rung 4), first occurrence. A brief's verify step must be side-effect-free on real state: point the tool at a throwaway (`CLAUDE_DOTFILES_DIR=<temp clone>` here), or defer the live run to the tech lead after review. If it recurs, add it to `rules/execution-workflow.md` "Briefs carry pasted state" as a sibling sentence.

## 2026-09-25 [P2] parse: an approximate day price ("~11.1 units") exported as null units and hid the dashboard's workload bar
