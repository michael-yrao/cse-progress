...
- [D&C coding gap](project_dandc_coding_gap.md) — learner gets D&C conceptually but stalls CODING the recursion; unlock (Jul 25): leap-of-faith + uniform return contract + "strategy not framework, one fn divides down/merges up". 912 taught unrated Jul 25 → rated re-rep Jul 29
- [Concepts lane — port owed to cse-coach](project_concepts_lane_port_pending.md) — cse-progress added an SD `concepts/` lane Jul 25–26 (6 cards + Concept tracker rows + lane-② fallback); cse-coach lacks it entirely and its tier1 curriculum has NO quantitative-foundations topic (Zipf/p99/Little's/quorum/sketches all absent; bloom filters mis-filed as a building block). Port owed upstream
...
- [Ask for complexity](feedback_ask_complexity.md) — GATE: every rep state time+space each with an itemized why-clause (no skip); per-problem freebie then a repeat miss on the SAME problem caps it at 🟡 (RESOLVED Jul 24, supersedes the Jul 22 pin). Ledger + reference: [complexity_gotchas.md](../../docs/foundations/dsa/mastery/complexity_gotchas.md)
