...
- [Grounded solutions Sep 22-23](project_grounded_solutions.md) — the site's visualizers render the learner's code verbatim from `dashboard/showcase.json` (pick in `showcase.yml`, method-level, content anchors); the groundedness meter is held at 100% by CI; the judgement calls (76 removed, 1216/355 re-picked, 125 kept with a caveat) and open follow-ups. Sep 24: gained a sibling contract, `dashboard/bigo.yml` → `export_bigo.py` → `big-o.json`, for the site's Big-O Trainer.
...
- [Learn cheat-sheet plan](../../docs/cse-coach/learn_cheatsheet_plan.md) — SHIPPED Sep 23, 2026: progressiveoverflow.com/learn is keyed by technique and rendered from `dashboard/cheat-sheets.json`, generated by `scripts/build_cheatsheets.py` from the technique docs (contract enforced by `check_cheatsheets.py` in pre-commit). Route D (personal-miss layer) still open.
