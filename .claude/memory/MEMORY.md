...
- [Agent portability (OPEN)](project_agent_portability.md) — cse-coach on Copilot + Cursor; the plan file `docs/cse-coach/AGENT_PORTABILITY_PLAN.md` is the single status record — read its Status line for what is done; §0 there is the pickup procedure.
- [Multi-agent hub (Phase 1 shipped)](project_multi_agent_portability.md) — `AGENTS.md` is the always-on hub for every agent; `CLAUDE.md` is `@AGENTS.md` + Claude-only; every Copilot/Cursor claim is documented-not-verified until the Phase 3 live probe runs.
