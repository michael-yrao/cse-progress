...
comfort written at the build, never mutated). It splits the day into **built · done ·
...
**FLOOR** (it won't hide rows it couldn't price). It cross-checks each day header's stated units
against its rows.
