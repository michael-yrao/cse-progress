...
[![progress](https://progressiveoverflow.com/progress) 🔥 117-day streak · 🎓 20 graduated · 🏆 0 retired · 59/98 techniques](https://progressiveoverflow.com/progress)
<!-- progress-badge:end -->
<!-- The badge between these markers is regenerated by scripts/gamify.py (via the pre-commit
     hook). It links to the live honest-progress dashboard on progressiveoverflow.com.
     scripts/build_cheatsheets.py emits dashboard/cheat-sheets.json from the technique docs
     the same way — a generated dashboard file, not a second copy — but as a deliberate run
     (`python scripts/build_cheatsheets.py`), not a pre-commit side effect; it is the single
     source for progressiveoverflow.com/learn. -->
